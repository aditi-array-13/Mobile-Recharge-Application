...
<body>
			...
			<form action="RechargeServlet" method="post">
				<table class="table">
					...
					<tr>
						...
						<td><select id="service" name="service" class="form-select"  onchange="updatePlans()"
							required>
					...
					<tr>
						...
						<td><select id="plan" name="plan" class="form-select" required></select></td>
					</tr>
					<tr>
						<td><label for="amount">Recharge Amount:</label></td>
	...
	<script>
		function updatePlans() {
			...
			if (service === 'Idea') {
				plans = ['Plan 1 - 199', 'Plan 2 - 399', 'Plan 3 - 599'];
			} else if (service === 'Airtel') {
				plans = ['Plan A - 249', 'Plan B - 449', 'Plan C - 649'];
			} else if (service === 'Jio') {
				plans = ['Plan X - 299', 'Plan Y - 499', 'Plan Z - 799'];
			}

			plans.forEach(function(planText) {
				const option = document.createElement('option');
				option.value = planText.split(' - ')[1];
				option.textContent = planText;
				plan.appendChild(option);
			});
		}

		window.onload = updatePlans;
	</script>
	<script
